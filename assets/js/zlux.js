...
function popup(status,message,style = { align:'',size:'md' })
{
    if(status)
    {
        return /*html*/`
        <div class="modal active modal-${style.size}" id="popup">
        <a href="#close" class="modal-overlay" aria-label="Close"></a>
        <div class="modal-container">
          <div class="modal-header">
            <a href="#close" class="btn btn-clear float-right" aria-label="Close"></a>
          ...
          <div class="modal-body">
            <div class="content" style="text-align:${style.align}">
                <span>${message}</span>
            </div>
          </div>
          <div class="modal-footer" >
...
function storage(objectname)
{
    return {
        insert:function(data,callback)
        {
            ...
            let data_store = []
            let store = localStorage.getItem(objectname)
            if(store == null)
            {
                data_store.push(data)
                data_store = JSON.stringify(data_store)
                localStorage.setItem(objectname,data_store)
                return (callback != undefined ) ? callback(success_data) : true
            }
            try {
                data_store = JSON.parse(store)
                data_store.push(data)
                localStorage.setItem(objectname,JSON.stringify(data_store))
                return (callback != undefined)?callback(success_data):true
            } catch (error) {
                fail_data.error = error
                return (callback != undefined)?callback(success_data):true
            }
        ...
        updateByIndexOf(index,value)
        {
            let store = localStorage.getItem(objectname)
                store = JSON.parse(store)
                store[index] = value 
                localStorage.setItem(objectname,JSON.stringify(store))
            return true
        }
    }
}